...
export const deepExtend = function (...objects: any[]): any {
  ...
  export function getDeepFromObject(object = {}, name: string, defaultValue?: any) {
    const keys = name.split('.'); console.log(keys); console.log('name', name); console.log('object', object); console.log('defaultValue', defaultValue);
    // clone the object
...
export function generateCode(str: string) {
         const code = str + Math.floor(Math.random() * (9999 - 1000 + 1) + 1000);
         return code;
}

export function generateStoreCode() {
  const storeCode: string = 'shop5312';
  return storeCode;
}

export function processQrCode(str: string) {
